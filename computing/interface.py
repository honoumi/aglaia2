    ...
    try:
        ...
        if 'check_time' in filt:
            ...
            if 'longer' in time_dict:
                q = q.filter(
                    expire_time__gt=date.today() + timedelta(days=time_dict['longer']))
        ...
        if 'check_time' in exclude:
            ...
            if ('longer' in time_dict) and ('shorter' in time_dict):
                q = q.exclude(Q(expire_time__gt=date.today() + timedelta(days=time_dict['longer'])) &
                              Q(expire_time__lte=date.today() + timedelta(days=time_dict['shorter'])))
