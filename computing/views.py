...
def do_return_request(request):
    try:
        ...
        if not (comp.status == BORROWED_KEY or comp.status == MODIFY_APPLY_KEY):
            return HttpResponse('denied')
        packed_update_computing(request, id, {'status': RETURNING_KEY}, log=get_comp_ret_log())
...
def do_modif_request(request):
    try:
        ...
        packed_update_computing(request, id,
            {'status': MODIFY_APPLY_KEY, 'note': post['reason']}, log=get_comp_modif_log())
...
def do_approve_borrow(request):
    try:
        ...
        return HttpResponseRedirect(reverse('computing.views.show_comp_verify'))
    except Exception as e:
        return show_message(request, 'Something wrong when approve computing resource: '+e.__str__())
...
def do_disapprove_borrow(request):
    try:
        ...
        return show_message(request, 'Something wrong when disapprove computing resource: '+e.__str__())
...
def do_approve_return(request):
    try:
        ...
        return show_message(request, 'Something wrong when approve return: '+e.__str__())
...
def do_approve_modify(request):
    try:
        ...
        return show_message(request, 'Something wrong when approve modify: '+e.__str__())
...
def do_disapprove_modify(request):
    try:
        ...
        return show_message(request, 'Something wrong when disapprove modify: '+e.__str__())
